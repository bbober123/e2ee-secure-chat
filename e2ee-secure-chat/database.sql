...
-- ---------------------------------------------------------------------
-- 5c. Reakcje emoji pod wiadomościami
-- ---------------------------------------------------------------------
-- UWAGA (świadomy kompromis, jak przy message.status): w przeciwieństwie
-- do treści wiadomości (ciphertext/nonce/header), same reakcje (👍❤️😂...)
-- trzymamy jawnym tekstem po stronie serwera, NIE przez Double Ratchet.
-- Powód: reakcja musi dotrzeć do WSZYSTKICH uczestników (w tym w grupach),
-- a każda para uczestników ma osobną sesję ratchetu / osobny Sender Key -
-- zaszyfrowanie jednej reakcji dla wszystkich odbiorców wymagałoby wysłania
-- jej jako osobnej, pełnoprawnej wiadomości do każdego z osobna. Uznaliśmy,
-- że dla samego emoji (bez treści rozmowy) to nieopłacalny kompromis -
-- serwer widzi WYŁĄCZNIE które emoji i kto je dodał pod którą wiadomością,
-- nigdy treść samej wiadomości.
...
DROP POLICY IF EXISTS "Users can read reactions in their conversations" ON public.message_reactions;
...
-- WITH CHECK weryfikuje conversation_id NIEZALEŻNIE od tego, co poda klient -
-- musi się zgadzać z prawdziwą konwersacją danej wiadomości, inaczej ktoś
-- mógłby podpiąć reakcję do wiadomości w konwersacji, do której nie należy,
-- podając fałszywe conversation_id z konwersacji, do której akurat należy.
DROP POLICY IF EXISTS "Users can add their own reaction" ON public.message_reactions;
CREATE POLICY "Users can add their own reaction" ON public.message_reactions
...
CREATE POLICY "Users can remove their own reaction" ON public.message_reactions
...
-- Ekwipunek ze skrzynek - przedmioty, które gracz zdecydował się ZACHOWAĆ
-- zamiast od razu "sprzedać" za żetony (żeby móc się pochwalić znajomym).
-- Limit 30 sztuk na konto pilnowany zarówno w RPC (poniżej) jak i w UI.
CREATE TABLE IF NOT EXISTS public.casino_inventory (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    user_id UUID NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
    ...
    case_title TEXT,
    -- Migawka wyglądu ikony (znacznik <img>/<svg> albo emoji) w chwili
    -- zdobycia - żeby przedmiot w ekwipunku wyglądał tak samo nawet jeśli
    -- admin później zmieni albo usunie tę broń z casino_weapons.
    icon_snapshot TEXT,
    obtained_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);
ALTER TABLE public.casino_inventory ADD COLUMN IF NOT EXISTS icon_snapshot TEXT;

CREATE INDEX IF NOT EXISTS idx_casino_inventory_user ON public.casino_inventory(user_id);

ALTER TABLE public.casino_inventory ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users manage only their own inventory" ON public.casino_inventory;
CREATE POLICY "Users manage only their own inventory" ON public.casino_inventory
    FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- Dodaje przedmiot do ekwipunku (atomowo, z twardym limitem 30 sztuk na
-- koncie - pilnowane w bazie, nie tylko w UI, żeby dwa szybkie kliknięcia
-- albo dwie karty przeglądarki nie mogły ominąć limitu).
-- DROP starej sygnatury (5 parametrów, bez icon_snapshot) - CREATE OR REPLACE
-- nie pozwala zmienić listy parametrów istniejącej funkcji.
DROP FUNCTION IF EXISTS public.casino_keep_item(TEXT, TEXT, TEXT, BIGINT, TEXT);

CREATE OR REPLACE FUNCTION public.casino_keep_item(
    p_item_name TEXT, p_weapon_key TEXT, p_rarity_id TEXT, p_value BIGINT, p_case_title TEXT, p_icon_snapshot TEXT DEFAULT NULL
)
RETURNS UUID AS $$
DECLARE
    current_count INT;
    new_id UUID;
BEGIN
    SELECT count(*) INTO current_count FROM public.casino_inventory WHERE user_id = auth.uid();
    IF current_count >= 30 THEN
        RAISE EXCEPTION 'inventory full';
    END IF;

    INSERT INTO public.casino_inventory (user_id, item_name, weapon_key, rarity_id, value, case_title, icon_snapshot)
    VALUES (auth.uid(), p_item_name, p_weapon_key, p_rarity_id, p_value, p_case_title, p_icon_snapshot)
...
GRANT EXECUTE ON FUNCTION public.casino_keep_item(TEXT, TEXT, TEXT, BIGINT, TEXT, TEXT) TO authenticated;

-- Sprzedaje przedmiot z ekwipunku z powrotem za żetony - atomowo usuwa
-- wiersz i dolicza żetony w jednej transakcji.
CREATE OR REPLACE FUNCTION public.casino_sell_item(p_item_id UUID)
RETURNS BIGINT AS $$
DECLARE
    item_value BIGINT;
    new_balance BIGINT;
BEGIN
    DELETE FROM public.casino_inventory
    WHERE id = p_item_id AND user_id = auth.uid()
    RETURNING value INTO item_value;
    ...
    UPDATE public.casino_balances
    SET chips = chips + item_value, updated_at = now()
    WHERE user_id = auth.uid()
    RETURNING chips INTO new_balance;
...
GRANT EXECUTE ON FUNCTION public.casino_sell_item(UUID) TO authenticated;

-- ---------------------------------------------------------------------
-- 9a2. Giełda — handel kryptowalutami za żetony kazyna (ceny prawdziwe
-- z CoinGecko po stronie klienta, ale saldo/portfel/historia w bazie -
-- tak samo jak reszta kazyna: bez wartości pieniężnej, bez wypłat).
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.casino_crypto_holdings (
    user_id UUID NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
    coin_id TEXT NOT NULL,
    symbol TEXT NOT NULL,
    name TEXT NOT NULL,
    amount NUMERIC(24,8) NOT NULL DEFAULT 0 CHECK (amount >= 0),
    -- Suma żetonów wydanych (bez prowizji) na obecnie posiadaną ilość -
    -- używana wyłącznie do liczenia zysku/straty na pozycji w UI.
    cost_basis BIGINT NOT NULL DEFAULT 0 CHECK (cost_basis >= 0),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
    PRIMARY KEY (user_id, coin_id)
);

ALTER TABLE public.casino_crypto_holdings ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users manage only their own crypto holdings" ON public.casino_crypto_holdings;
CREATE POLICY "Users manage only their own crypto holdings" ON public.casino_crypto_holdings
    FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS public.casino_crypto_trades (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    user_id UUID NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
    coin_id TEXT NOT NULL,
    symbol TEXT NOT NULL,
    ...
    total BIGINT NOT NULL, -- żetony faktycznie obciążone (buy) / uznane (sell), z prowizją
    created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

CREATE INDEX IF NOT EXISTS idx_casino_crypto_trades_user ON public.casino_crypto_trades(user_id, created_at DESC);

ALTER TABLE public.casino_crypto_trades ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users manage only their own crypto trades" ON public.casino_crypto_trades;
CREATE POLICY "Users manage only their own crypto trades" ON public.casino_crypto_trades
    FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- Atomowa transakcja kup/sprzedaj: sprawdza i zmienia saldo żetonów,
-- posiadaną ilość kryptowaluty i dopisuje wiersz historii - wszystko w
-- jednej transakcji SQL, żeby dwa szybkie kliknięcia nie mogły wydać
-- więcej żetonów niż jest na koncie ani sprzedać więcej niż się posiada.
DROP FUNCTION IF EXISTS public.casino_crypto_trade(TEXT, TEXT, TEXT, TEXT, NUMERIC, NUMERIC);

CREATE OR REPLACE FUNCTION public.casino_crypto_trade(
    p_side TEXT, p_coin_id TEXT, p_symbol TEXT, p_name TEXT, p_amount NUMERIC, p_price NUMERIC
)
RETURNS jsonb AS $$
DECLARE
    gross BIGINT;
...
BEGIN
    ...
    gross := GREATEST(ROUND(p_amount * p_price), 0);
    fee := GREATEST(ROUND(gross * 0.001), 1); -- min. 1 żeton prowizji
    ...
    IF p_side = 'buy' THEN
        ...
        UPDATE public.casino_balances
        SET chips = chips - grand, updated_at = now()
        WHERE user_id = auth.uid() AND chips >= grand
        RETURNING chips INTO new_chips;

        IF new_chips IS NULL THEN
            RAISE EXCEPTION 'insufficient chips';
        END IF;

        INSERT INTO public.casino_crypto_holdings (user_id, coin_id, symbol, name, amount, cost_basis)
        VALUES (auth.uid(), p_coin_id, p_symbol, p_name, p_amount, gross)
        ON CONFLICT (user_id, coin_id) DO UPDATE
        SET amount = public.casino_crypto_holdings.amount + EXCLUDED.amount,
            cost_basis = public.casino_crypto_holdings.cost_basis + EXCLUDED.cost_basis,
            symbol = EXCLUDED.symbol, name = EXCLUDED.name, updated_at = now()
        RETURNING amount INTO new_amount;

        INSERT INTO public.casino_crypto_trades (user_id, coin_id, symbol, name, side, amount, price, fee, total)
        VALUES (auth.uid(), p_coin_id, p_symbol, p_name, 'buy', p_amount, p_price, fee, grand);

        RETURN jsonb_build_object('new_chips', new_chips, 'new_amount', new_amount);
    ELSE
        SELECT amount, cost_basis INTO held, held_cost
        FROM public.casino_crypto_holdings WHERE user_id = auth.uid() AND coin_id = p_coin_id
        FOR UPDATE;

        IF held IS NULL OR held < p_amount THEN
            RAISE EXCEPTION 'insufficient holdings';
        END IF;

        grand := gross - fee;
        new_amount := held - p_amount;
        -- proporcjonalna część kosztu zakupu zostaje na pozostałej pozycji
        new_cost := GREATEST(ROUND(held_cost * (new_amount / NULLIF(held, 0))), 0);

        IF new_amount <= 0.00000001 THEN
            DELETE FROM public.casino_crypto_holdings WHERE user_id = auth.uid() AND coin_id = p_coin_id;
            new_amount := 0;
        ELSE
            UPDATE public.casino_crypto_holdings
            SET amount = new_amount, cost_basis = new_cost, updated_at = now()
        ...
        INSERT INTO public.casino_crypto_trades (user_id, coin_id, symbol, name, side, amount, price, fee, total)
        VALUES (auth.uid(), p_coin_id, p_symbol, p_name, 'sell', p_amount, p_price, fee, grand);
...
GRANT EXECUTE ON FUNCTION public.casino_crypto_trade(TEXT, TEXT, TEXT, TEXT, NUMERIC, NUMERIC) TO authenticated;

-- ---------------------------------------------------------------------
-- 9b. Panel admina — broń i skrzynki w kasynie jako dane w bazie (nie na
-- sztywno w kodzie), żeby dało się je edytować z panelu bez wdrażania.
-- ---------------------------------------------------------------------
-- ALTER, nie tylko CREATE TABLE - kolumna musi dojść nawet jeśli users
-- już istniało (CREATE TABLE IF NOT EXISTS pomija istniejące tabele).
ALTER TABLE public.users ADD COLUMN IF NOT EXISTS is_admin BOOLEAN NOT NULL DEFAULT false;

-- Pomocnicza funkcja do polityk RLS poniżej i do sprawdzania z klienta
-- (`supabase.rpc('is_admin')`), czy pokazać zakładkę "Panel admina".
...
CREATE TABLE IF NOT EXISTS public.casino_weapons (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    name TEXT NOT NULL UNIQUE,
    rarity_id TEXT NOT NULL CHECK (rarity_id IN ('mil-spec', 'restricted', 'classified', 'covert', 'rare-gold')),
    -- Obrazek wgrany przez admina, jako data: URL (base64) - prościej niż
    -- konfigurować osobny bucket w Supabase Storage. Jeśli NULL, front-end
    -- używa domyślnej ikony SVG (jeśli nazwa akurat pasuje do wbudowanego
    -- zestawu) albo emoji jako ostatecznego fallbacku.
    icon_data TEXT,
    active BOOLEAN NOT NULL DEFAULT true,
    created_by UUID REFERENCES public.users(id) ON DELETE SET NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);
ALTER TABLE public.casino_weapons ADD COLUMN IF NOT EXISTS icon_data TEXT;
ALTER TABLE public.casino_weapons ADD COLUMN IF NOT EXISTS active BOOLEAN NOT NULL DEFAULT true;

CREATE TABLE IF NOT EXISTS public.casino_case_defs (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
...
ALTER TABLE public.casino_case_defs ADD COLUMN IF NOT EXISTS active BOOLEAN NOT NULL DEFAULT true;
ALTER TABLE public.casino_case_defs ADD COLUMN IF NOT EXISTS sort_order INT NOT NULL DEFAULT 0;

ALTER TABLE public.casino_weapons ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.casino_case_defs ENABLE ROW LEVEL SECURITY;

-- Każdy zalogowany może CZYTAĆ (musi widzieć broń/skrzynki, żeby otwierać skrzynki).
DROP POLICY IF EXISTS "Everyone can view weapons" ON public.casino_weapons;
CREATE POLICY "Everyone can view weapons" ON public.casino_weapons
    FOR SELECT USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Everyone can view cases" ON public.casino_case_defs;
CREATE POLICY "Everyone can view cases" ON public.casino_case_defs
    FOR SELECT USING (auth.uid() IS NOT NULL);

-- Tylko admin może DODAWAĆ/EDYTOWAĆ/USUWAĆ.
DROP POLICY IF EXISTS "Admins insert weapons" ON public.casino_weapons;
CREATE POLICY "Admins insert weapons" ON public.casino_weapons FOR INSERT WITH CHECK (public.is_admin());
DROP POLICY IF EXISTS "Admins update weapons" ON public.casino_weapons;
CREATE POLICY "Admins update weapons" ON public.casino_weapons FOR UPDATE USING (public.is_admin()) WITH CHECK (public.is_admin());
DROP POLICY IF EXISTS "Admins delete weapons" ON public.casino_weapons;
CREATE POLICY "Admins delete weapons" ON public.casino_weapons FOR DELETE USING (public.is_admin());

DROP POLICY IF EXISTS "Admins insert cases" ON public.casino_case_defs;
CREATE POLICY "Admins insert cases" ON public.casino_case_defs FOR INSERT WITH CHECK (public.is_admin());
DROP POLICY IF EXISTS "Admins update cases" ON public.casino_case_defs;
CREATE POLICY "Admins update cases" ON public.casino_case_defs FOR UPDATE USING (public.is_admin()) WITH CHECK (public.is_admin());
DROP POLICY IF EXISTS "Admins delete cases" ON public.casino_case_defs;
CREATE POLICY "Admins delete cases" ON public.casino_case_defs FOR DELETE USING (public.is_admin());

-- Dane startowe - to, co wcześniej było na sztywno w cases.js (16 broni +
-- 8 przedmiotów "złotych" + 8 skrzynek), żeby po migracji nic nie zniknęło.
-- ON CONFLICT DO NOTHING - bezpieczne przy wielokrotnym uruchomieniu.
INSERT INTO public.casino_weapons (name, rarity_id) VALUES
    ('AK-47', 'mil-spec'), ('M4A4', 'mil-spec'), ('Glock-18', 'mil-spec'), ('USP', 'mil-spec'),
    ('M4A1-S', 'restricted'), ('MP7', 'restricted'), ('P90', 'restricted'), ('P250', 'restricted'),
    ('AWP', 'classified'), ('Desert Eagle', 'classified'), ('Five-SeveN', 'classified'),
    ('Sawed-Off', 'covert'), ('Negev', 'covert'), ('MAC-10', 'covert'),
    ('Karambit', 'rare-gold'), ('Butterfly', 'rare-gold'), ('Bayonet', 'rare-gold'), ('Gut Knife', 'rare-gold'),
    ('Stiletto', 'rare-gold'), ('Ursus', 'rare-gold'), ('Rękawice Bojowe', 'rare-gold'), ('Rękawice', 'rare-gold')
ON CONFLICT (name) DO NOTHING;

INSERT INTO public.casino_case_defs (slug, title, price, seed, sort_order) VALUES
    ('street', 'Skrzynka Uliczna', 50, 0, 1),
    ('vault', 'Skrzynka Skarbca', 100, 11, 2),
    ('phantom', 'Skrzynka Widmo', 200, 23, 3),
    ('neon', 'Skrzynka Neonowa', 350, 37, 4),
    ('obsidian', 'Skrzynka Obsydianowa', 600, 51, 5),
    ('crimson', 'Skrzynka Purpurowa', 1000, 67, 6),
    ('titan', 'Skrzynka Tytanowa', 1750, 83, 7),
    ('legend', 'Skrzynka Legendarna', 3000, 101, 8)
...
-- Reakcje muszą się pojawiać na żywo u wszystkich uczestników konwersacji
-- (patrz ChatApp.subscribeToReactions w chat/reactions.js).
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_publication_tables
