...
export class Roulette {
            ...
            <div class="roulette-layout">
                ...
                <div class="roulette-controls">
                    <div class="roulette-chip-select">
                        Żeton: ${[5, 25, 100, 500].map(v => `<button class="chip-btn ${v === (this.selectedChip || 25) ? 'chip-selected' : ''}" data-chip="${v}">${v}</button>`).join('')}
                    </div>
                    <div class="roulette-staked">Postawione: <strong>${this.totalStaked}</strong> żetonów</div>
    ...
    _wire() {
        this.el.querySelectorAll('[data-chip]').forEach(btn => {
            btn.addEventListener('click', () => { this.selectedChip = parseInt(btn.dataset.chip, 10); this.render(); });
        });
