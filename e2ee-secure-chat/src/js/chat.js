...
export class ChatApp {
    ...
    static async sendMessage(text) {
        ...
        try {
            ...
            const trust = await KeyTrustStore.verify(window.CURRENT_USER.id, contactId, window.APP_MODE, keyStr);
            if (trust.status === 'changed') {
                ...
                KeyTrustStore.trustNewKey(window.CURRENT_USER.id, contactId, window.APP_MODE, trust.fingerprint);
            }
    ...
    static subscribeToTyping(convId) {
        ...
        this.typingChannel = supabase.channel(`typing-${convId}`, { config: { broadcast: { self: false } } })
            .on('broadcast', { event: 'typing' }, (payload) => {
                ...
                if (!userId || userId === window.CURRENT_USER.id) return;
    ...
    static broadcastTyping() {
        ...
        this.typingChannel.send({
            type: 'broadcast',
            event: 'typing',
            payload: { userId: window.CURRENT_USER.id, isTyping: true }
        });
    }
    ...
    static stopTypingBroadcast() {
        ...
        this.typingChannel.send({
            type: 'broadcast',
            event: 'typing',
            payload: { userId: window.CURRENT_USER.id, isTyping: false }
        });
    }
