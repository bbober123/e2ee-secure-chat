...
export class ChatApp {
    ...
    static async init() {
        try {
            ...
            if (myAvatarImg) {
                myAvatarImg.src = data.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${window.CURRENT_USER.id}`;
            }
        } catch (err) {
    ...
    static async loadContacts() {
        const { data, error } = await supabase.from('contacts').select('*, contact_user:contact_user_id(username, public_key_real, public_key_fake)');
        ...
        data.forEach(c => {
            ...
            this.contacts.set(c.contact_user_id, {
                ...
                avatar: window.APP_MODE === 'fake' ? (c.fake_avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${nickname}Fake`) : (c.real_avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${nickname}`)
            });
        });
    }
    ...
    static async decryptMessageRow(msg) {
        ...
        const avatar = isMe ? `https://api.dicebear.com/7.x/avataaars/svg?seed=${window.CURRENT_USER.id}` : (contact ? contact.avatar : '');
    ...
    static async sendMessage(text) {
        ...
        try {
            ...
            const tempMsg = {
                ...
                authorName: 'Ja',
                avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${window.CURRENT_USER.id}`,
