...
import { IdentityVault, publishBundle } from './prekeys.js';

export const keyManager = new KeyManager();
// Tryb ('real'/'fake') i użytkownik żyją WYŁĄCZNIE w AppState (domknięcie modułu),
// nigdy na window - patrz komentarz w state.js.
...
export class AuthManager {
    ...
    static async register(email, username, passwordReal, passwordFake) {
        ...
        // Generuj OSOBNE tożsamości X3DH (IK/SPK/OPK) dla real i fake — tak samo jak istniejące
        // pary RSA — zanim mamy jeszcze saltReal/saltFake, więc na razie tylko w RAM.
        const identityVaultRealPlain = await IdentityVault.create();
        const identityVaultFakePlain = await IdentityVault.create();

        const vaultData = await DoubleVault.createUser(passwordReal, fakePass);
        const identityVaultReal = await IdentityVault.encrypt(identityVaultRealPlain, vaultData.passwordKeyReal);
        const identityVaultFake = await IdentityVault.encrypt(identityVaultFakePlain, vaultData.passwordKeyFake);
        vaultData.deviceRecord.encrypted_prekey_vault_real = JSON.stringify(identityVaultReal);
        vaultData.deviceRecord.encrypted_prekey_vault_fake = JSON.stringify(identityVaultFake);
        ...
        // Publikuj klucze publiczne (IK/SPK/OPK) obu tożsamości, żeby inni użytkownicy mogli
        // od razu zainicjować X3DH z tym kontem. Nieblokujące dla samej rejestracji, ale
        // logujemy błąd - bez tego kontakt nie będzie mógł napisać pierwszej wiadomości.
        try {
            await publishBundle(userId, 'real', identityVaultRealPlain);
            await publishBundle(userId, 'fake', identityVaultFakePlain);
        } catch (e) {
            console.error('Nie udało się opublikować kluczy X3DH', e);
        }
    ...
    static async loginAndUnlock(email, password) {
        ...
        AppState.setMode(unlockResult.mode);
        AppState.setUser(authData.user);
    ...
    static async logout() {
        ...
        if (AppState.getUser()) {
            AccountSwitcher.remove(AppState.getUser().id);
        }
    ...
    static lockDevice() {
        ...
        AppState.setMode(null);
        this.clearAutoLockTimer();
        // Nie importujemy tu bezpośrednio ChatApp/CallManager (uniknięcie zależności
        // cyklicznej auth.js <-> chat.js) - zamiast tego wysyłamy zdarzenie, na które
        // main.js reaguje kończąc ewentualne aktywne połączenie głosowe/wideo, żeby
        // po zablokowaniu urządzenia mikrofon/kamera nie zostały otwarte w tle.
