...
export const UiHandlersMixin = {
    setupUIHandlers() {
        ...
        document.getElementById('messages-container').addEventListener('click', async (e) => {
            ...
            if (reactBtn) {
                UI.closeReactionPicker();
                this.toggleReaction(reactBtn.dataset.reactToggle, reactBtn.dataset.reactEmoji);
                return;
            }

            const openPickerBtn = e.target.closest('[data-react-open]');
            if (openPickerBtn) {
                UI.toggleReactionPicker(openPickerBtn.dataset.reactOpen);
                return;
            }

            // Klik gdziekolwiek indziej w liście wiadomości zamyka otwarty picker reakcji.
            UI.closeReactionPicker();
        });
