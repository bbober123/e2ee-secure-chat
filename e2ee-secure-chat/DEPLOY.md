...
## Nowość: gra "Giełda" w kazynie
Kazyno ma nową grę — Giełda: handel kryptowalutami (ceny live z CoinGecko,
publiczne API bez klucza) za żetony z tego samego portfela co reszta gier
(`casino_balances`). Portfel krypto i historia transakcji siedzą w bazie
(`casino_crypto_holdings`, `casino_crypto_trades`), nie w localStorage - więc
działają tak samo na telefonie i komputerze.

Jeśli baza była już wcześniej wdrożona (masz `casino_balances` i
`casino_adjust_chips` z sekcji "Kazyno" w `database.sql`), wystarczy uruchomić
w SQL Editorze **tylko** nowy plik `migration_casino_exchange.sql` — jest
addytywny (`IF NOT EXISTS`/`CREATE OR REPLACE` wszędzie), bezpieczny do
uruchomienia wielokrotnie. Przy zupełnie nowym wdrożeniu ten fragment jest
już częścią pełnego `database.sql`, więc osobny krok nie jest potrzebny.

CSP w `index.html` ma już dopisany `https://api.coingecko.com` do
`connect-src` — bez tego przeglądarka zablokuje pobieranie cen.
